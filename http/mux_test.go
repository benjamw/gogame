...
type routeTest struct {
	...
	extensions     []string          // the expected route extensions
...
type ResponseRecorder struct {
	Code      int           // Code is the HTTP response code from WriteHeader
	HeaderMap http.Header   // HeaderMap is the HTTP response headers
	Body      *bytes.Buffer // Body if non-nil, the bytes.Buffer to append written data to
	Flushed   bool
}
...
func TestRouteMatchers(t *testing.T) {
	...
	match := func(shouldMatch bool) {
		...
		if matched != shouldMatch {
			t.Errorf("Expected: %v\nGot: %v\nRequest: %v %v", shouldMatch, matched, request.Method, path)
		}
...
func TestUrlBuilding(t *testing.T) {
	...
	for _, v := range urlBuildingTests {
		...
		path := u.String()
		if path != v.url {
			t.Errorf("expected %v, got %v", v.url, path)
			/*
				reversePath := ""
				reverseHost := ""
	...
	u, _ := router.Get("article").URL("category", "technology", "id", "42")
	expected := "/articles/technology/42"
	if u.String() != expected {
		t.Errorf("Expected %v, got %v", expected, u.String())
	}
}
...
func TestMatchedRouteName(t *testing.T) {
	...
	path := "http://www.example.com/products/"
...
func TestSubRouting(t *testing.T) {
	...
	if builtURL != path {
		t.Errorf("Expected %q, got %q.", path, builtURL)
	}
}
...
// TestNewRegexp for the new regexp library, still not available in stable Go.
...
func TestExtensions(t *testing.T) {
	tests := []routeTest{
		{
			title:       "Extensions route, match HTML",
			route:       new(Route).Extensions("HTML", "JPG"),
			request:     newRequest("GET", "http://localhost/test.html"),
			vars:        map[string]string{},
			host:        "",
			path:        "",
		...
		{
			title:       "Extensions route, match JPG",
			route:       new(Route).Extensions("HTML", "JPG"),
			request:     newRequest("GET", "http://localhost/test.jpg"),
			vars:        map[string]string{},
			host:        "",
			path:        "",
			extensions:  []string{"HTML", "JPG"},
		...
		{
			title:       "Extensions route, bad extension",
			route:       new(Route).Extensions("HTML", "JPG"),
			request:     newRequest("GET", "http://localhost/test.gif"),
			vars:        map[string]string{},
			host:        "",
			path:        "",
			extensions:  []string{"HTML", "JPG"},
		...
		{
			title:       "Extensions route, no extension",
			route:       new(Route).Extensions("HTML", "JPG"),
		...
		{
			title:       "Route without extensions",
			route:       new(Route),
			request:     newRequest("GET", "http://localhost/test.html"),
			vars:        map[string]string{},
			host:        "",
...
func TestRoutePriority(t *testing.T) {
	...
	func3 := func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, "func3")
	}

	r := NewRouter()
	r.Path("/foo").Priority(2).HandlerFunc(func2)
	r.Path("/foo").Priority(1).HandlerFunc(func1)
	r.Path("/foo").Priority(3).HandlerFunc(func3)
	...
	if res.Body.String() != "func1" {
		t.Errorf("TestRoutePriority did not return expected response. Wanted: func1. Got: %s", res.Body.String())
	}
}

func TestRouteStackedPriority(t *testing.T) {
	func1 := func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, "func1")
	}
	func2 := func(w http.ResponseWriter, r *http.Request) {
	...
	r.Path("/foo").Priority(2).HandlerFunc(func2)
	r.Path("/foo").Priority(1).HandlerFunc(func1)
	r.Path("/foo").Priority(1).HandlerFunc(func3)
	...
	if res.Body.String() != "func1" {
		t.Errorf("TestRouteStackedPriority did not return expected response. Wanted: func1. Got: %s", res.Body.String())
	}
}

func TestRouteDefaultPriority(t *testing.T) {
	func1 := func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, "func1")
	}
	func2 := func(w http.ResponseWriter, r *http.Request) {
	...
	if res.Body.String() != "func1" {
		t.Errorf("TestRouteDefaultPriority did not return expected response. Wanted: func1. Got: %s", res.Body.String())
	}
}
...
func TestCombinedRoutes(t *testing.T) {
	...
	func4 := func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, "func4")
	}
	func5 := func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, "func5")
	}
	func6 := func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, "func6")
	}

	r := NewRouter()
	r.PathPrefix("/foo").HandlerFunc(func2)
	r.Path("/foo").Priority(1).HandlerFunc(func1)
	r.Path("/foo").Priority(3).HandlerFunc(func2)
	...
	if res.Body.String() != "func1" {
		t.Errorf("TestCombinedRoutes #01 did not return expected response. Wanted: func1. Got: %s", res.Body.String())
	}

	r = NewRouter()
	q := r.Priority(10).Subrouter() // low priority
	s := r.Priority(2).Subrouter()  // high priority

	q.PathPrefix("/foo").HandlerFunc(func2)
	q.PathPrefix("/").HandlerFunc(func3)
	q.PathPrefix("/bar").HandlerFunc(func4)

	s.PathPrefix("/foo").HandlerFunc(func1)
	s.PathPrefix("/").HandlerFunc(func5)
	s.PathPrefix("/").Extensions("html").HandlerFunc(func6)
	...
	if res.Body.String() != "func1" {
		t.Errorf("TestCombinedRoutes #02 did not return expected response. Wanted: func1. Got: %s", res.Body.String())
	}
	...
	if res.Body.String() != "func4" {
		t.Errorf("TestCombinedRoutes #02 did not return expected response. Wanted: func4. Got: %s", res.Body.String())
	}

	res = NewRecorder()
	req, _ = http.NewRequest("GET", "/bar.html", nil)

	r.ServeHTTP(res, req)

	if res.Body.String() != "func6" {
		t.Errorf("TestCombinedRoutes #02 did not return expected response. Wanted: func6. Got: %s", res.Body.String())
	}
}
...
func TestStrictSlash(t *testing.T) {
	...
	tests := []routeTest{
		...
		},
		{
			title:          "Ignore StrictSlash for path prefix",
			route:          r.NewRoute().PathPrefix("/static/"),
			request:        newRequest("GET", "http://localhost/static"),
			vars:           map[string]string{},
			host:           "",
			path:           "/static/",
			shouldMatch:    false,
...
// TestPanicOnCapturingGroups, see: https://github.com/gorilla/mux/issues/200
...
func testExtensions(t *testing.T, test routeTest) {
	route := test.route
	exts, _ := route.GetExtensions()
	if strings.Join(exts, ",") != strings.Join(test.extensions, ",") {
		t.Errorf("(%v) GetExtensions not equal: expected %v, got %v", test.title, test.extensions, exts)
	}
}
...
// TestSubrouterHeader, see: https://plus.google.com/101022900381697718949/posts/eWy6DjFJ6uW
...
func TestNoMatchMethodErrorHandler(t *testing.T) {
	...
	if match.MatchErr != ErrMethodMismatch {
		t.Errorf("Should get ErrMethodMismatch error, got: %v", match.MatchErr)
	}

	resp := NewRecorder()
	r.ServeHTTP(resp, req)
	if resp.Code != 405 {
		t.Errorf("Expecting code %v, got: %v", 405, resp.Code)
	}
